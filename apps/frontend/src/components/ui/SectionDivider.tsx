...
const SectionDivider: React.FC<SectionDividerProps> = ({
  ...
  const getColorClass = () => {
    switch (color) {
      ...
      case 'gradient':
        return 'text-transparent bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 bg-clip-text';
      default:
        return 'text-blue-500';
    }
  };
  ...
  const renderLines = () => (
    <div className="relative h-16 w-full overflow-hidden">
      <div className="absolute inset-0 flex items-center">
        ...
        <motion.div
          animate={{ scaleX: 1 }}
          className="absolute left-1/4 h-0.5 w-1/2 bg-gradient-to-r from-purple-500 to-green-500 opacity-60"
          initial={{ scaleX: 0 }}
        ...
        {[25, 50, 75].map((position, i) => (
          <motion.div
            ...
            className="absolute h-3 w-3 rounded-full bg-blue-500 opacity-80"
    ...
    <div className="relative overflow-hidden py-8">
      ...
      {Array.from({ length: 6 }, (_, i) => (
        <motion.div
          ...
          className="absolute h-1 w-1 rounded-full bg-blue-400 opacity-40"
          key={i}
          style={{
            left: `${15 + i * 15}%`,
            top: `${20 + (i % 2) * 60}%`,
      ...
      {/* Contenu principal */}
      <div className="relative z-10">{renderVariant()}</div>
    </div>
  );
};
