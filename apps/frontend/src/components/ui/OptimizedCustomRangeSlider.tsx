...
export const OptimizedCustomRangeSlider: React.FC<
  OptimizedCustomRangeSliderProps
> = ({
  min,
  max,
  value,
  ...
  const updateValue = useCallback(
    (e: MouseEvent | React.MouseEvent, thumb: 'min' | 'max') => {
  ...
  const minPercentage = Math.max(
    0,
    Math.min(100, ((localValue[0] - min) / (max - min)) * 100)
  );
  const maxPercentage = Math.max(
    0,
    Math.min(100, ((localValue[1] - min) / (max - min)) * 100)
  );
  ...
  return (
    <div className={`w-full space-y-3 ${className}`}>
      ...
      <div className="relative px-3 py-4">
        {/* Slider Track */}
        <div
          className="relative h-2 cursor-pointer rounded-full bg-gray-700/50"
          ref={sliderRef}
        >
          {/* Active Range */}
          <div
          ...
          {/* Min Thumb */}
          <div
            className={`absolute h-5 w-5 cursor-grab rounded-full border-2 bg-white shadow-lg transition-all duration-200 active:cursor-grabbing ${
              isDragging === 'min'
                ? 'scale-125 border-blue-500 shadow-blue-500/50 shadow-lg'
                : 'border-gray-400 hover:scale-110 hover:border-blue-400'
            }`}
          ...
          <div
            className={`absolute h-5 w-5 cursor-grab rounded-full border-2 bg-white shadow-lg transition-all duration-200 active:cursor-grabbing ${
              isDragging === 'max'
                ? 'scale-125 border-purple-500 shadow-lg shadow-purple-500/50'
                : 'border-gray-400 hover:scale-110 hover:border-purple-400'
            }`}
