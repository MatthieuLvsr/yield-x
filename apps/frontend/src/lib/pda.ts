...
export const getDepositPDA = (
  ...
  return PublicKey.findProgramAddressSync(
    [Buffer.from('deposit'), strategy.toBuffer(), user.toBuffer()],
    PROGRAM_ID
  );
};

export const getYieldTokenMintPDA = (
  strategy: PublicKey
): [PublicKey, number] => {
  return PublicKey.findProgramAddressSync(
    [Buffer.from('yield_token'), strategy.toBuffer()],
    PROGRAM_ID
  );
};

// Fonction utilitaire pour dériver le PDA d'une stratégie avec APY
export const deriveStrategyPda = (
  tokenMint: PublicKey,
  apy: number,
  programId: PublicKey
): PublicKey => {
  ...
  const [strategyPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('strategy'), tokenMint.toBuffer(), apyBytes],
    programId
  );
  return strategyPda;
};
...
export const deriveStrategyTokenAccountPda = (
  ...
  const [strategyTokenAccountPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('strategy_token'), tokenMint.toBuffer(), apyBytes],
    programId
  );
