...
export const findStrategyForDeposit = (
  ...
  const strategyByAddress = strategies.find(strategy => 
    strategy.publicKey.toString() === deposit.strategyAddress
  );
  ...
  const strategyByToken = strategies.find(strategy => 
    strategy.token === tokenSymbol
  );
 ...
 */
export const fromTokenBaseUnits = (amount: bigint, decimals: number): number => {
...
export const convertStoredAmountToDisplayAmount = (
  ...
  const numAmount = typeof storedAmount === 'string' ? parseFloat(storedAmount) : storedAmount;
  const decimals = getTokenDecimals(tokenSymbol);
...
export const enrichDepositsWithStrategyInfo = (
  ...
  return deposits.map(deposit => {
    ...
    const tokenSymbol = strategy?.token || getTokenSymbolFromAddress(deposit.tokenAddress);
    const tokenDecimals = getTokenDecimals(tokenSymbol);
    
    // Convertir les montants stockés vers les montants d'affichage
    const displayAmount = convertStoredAmountToDisplayAmount(deposit.amount, tokenSymbol);
    const displayYieldAmount = convertStoredAmountToDisplayAmount(deposit.yieldAmount, tokenSymbol);
    ...
    return {
      ...
      formattedYieldAmount: formatTokenAmount(displayYieldAmount, tokenSymbol, true)
    };
  });
};
 ...
 */
export const calculateEnrichedStats = (enrichedDeposits: EnrichedUserDeposit[]) => {
  const totalsByToken: Record<string, { amount: number; yieldAmount: number; count: number }> = {};
...
export const getDisplayAmount = (deposit: EnrichedUserDeposit): string => {
  ...
  const displayAmount = convertStoredAmountToDisplayAmount(deposit.amount, deposit.tokenSymbol);
...
export const getDisplayYieldAmount = (deposit: EnrichedUserDeposit): string => {
  ...
  const displayYieldAmount = convertStoredAmountToDisplayAmount(deposit.yieldAmount, deposit.tokenSymbol);
 ...
 */
export const getDisplayAmountNumeric = (deposit: EnrichedUserDeposit): number => {
  return convertStoredAmountToDisplayAmount(deposit.amount, deposit.tokenSymbol);
};

/**
 * Obtient le montant de yield numérique d'affichage pour un dépôt
 */
export const getDisplayYieldAmountNumeric = (deposit: EnrichedUserDeposit): number => {
  return convertStoredAmountToDisplayAmount(deposit.yieldAmount, deposit.tokenSymbol);
};
